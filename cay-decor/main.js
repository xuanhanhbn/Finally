...
if (seeMore) {
  seeMore.onclick = () => {
    if (
      detaiContent.getAttribute("class") === "products-detai-content active"
    ) {
      ...
      seeMore.innerHTML =
        ' Thu gọn <i class="fa-solid fa-caret-up icon-up"></i> ';
    }
    ...
    if (valueinput) {
      arrelement = titleProducts.filter((data) => {
        return data
          .toLowerCase()
          .replace(/ /g, "")
          .startsWith(valueinput.toLowerCase().replace(/ /g, ""));
      });
      ...
      if (contaiProducts) {
        ...
        for (let i = 0; i < prodCard.length; i++) {
          let titleProd = prodCard[i].querySelector(
            ".card h5.card-title"
          ).innerText;
          ...
          if (
            titleProd
              .toLowerCase()
              .replace(/ /g, "")
              .indexOf(valueinput.toLowerCase().replace(/ /g, "")) > -1
          ) {
      ...
      if (
        btntabs2 &&
        btntabs2.getAttribute("class") === "btn-product-top active"
      ) {
        showproBtn2();
      }
      if (
        btntabs3 &&
        btntabs3.getAttribute("class") === "btn-product-top active"
      ) {
        showproBtn3();
      }
    }
...
function selectLi(e) {
  ...
  for (let i = 0; i < copyCard.length; i++) {
    ...
    if (
      textCatdCopy
        .toLowerCase()
        .replace(/ /g, "")
        .startsWith(e.innerText.toLowerCase().replace(/ /g, ""))
    ) {
  ...
  document.querySelector(
    ".js-search > p"
  ).innerHTML = ` <p>Kết quả được từ tìm thấy với từ khóa"<span class="keyword-search">${e.innerText}</span>"</p>`;
  showCard(copyData);
}
...
function showLiSearch(data) {
  ...
  if (ulSearch) ulSearch.innerHTML = htmlUl;
}
...
function showCard(data) {
  ...
  if (document.querySelector(".js-search")) contaiShowCard.innerHTML = htmlCard;
...
function dataCard(keywCard, valueinput) {
  if (document.querySelector(".js-index")) {
    ...
    for (let i = 0; i < arrTitlCard.length; i++) {
      ...
      if (
        keywtitlSearch.toLowerCase().replace(/ /g, "").startsWith(valueinput)
      ) {
        let parenkeyw =
          arrTitlCard[i].parentElement.parentElement.parentElement;
        let sale = "";
        ...
        document.querySelector(
          ".js-search > p"
        ).innerHTML = ` <p>Kết quả được từ tìm thấy với từ khóa"<span class="keyword-search">${valueinput}</span>"</p>`;
      } else {
        document.querySelector(
          ".js-search > p"
        ).innerHTML = ` <p> Không tìm thấy kết quả nào tìm thấy với từ khóa "<span class="keyword-search">${valueinput}</span>"</p>`;
      }
    }
...
function toast({ title = "", message = "", typeo = "", duration = 3000 }) {
  ...
  function renderProduct(data) {
    ...
    allTabs.forEach((tab) => {
      tab.onclick = function () {
        document
          .querySelector(".btn-product-top.active")
...
if (indexProdDetai) {
  ...
  btnOderCard.onclick = () => {
    ...
    let idProduct = document
      .querySelector(".title-pdoduct-detai")
      .getAttribute("id-data"); // get id-data
  ...
  function checkItem(objProduct, numqty) {
    if (
      arrItems.findIndex(
        (arrItem) => arrItem.idProduct === objProduct.idProduct
      ) == -1
    ) {
  ...
  imgSub.forEach((img) => {
    img.onclick = () => {
      document
        .querySelector(".sub-photo img.opacity")
  ...
  for (let i = 0; i < reduction2.length; i++) {
    ...
    reduction2[i].onclick = () => {
      ...
      if (num <= 0) {
        ...
        let reduc2 =
          reduction2[i].parentElement.parentElement.querySelector(
            ".item-into-money2"
          );
    ...
    more2[i].onclick = () => {
      ...
      let more2Click =
        more2[i].parentElement.parentElement.querySelector(".item-into-money2");
...
function totalCart() {
  ...
  resultTotal === 0
    ? (document.querySelector(".into-money").textContent = "0VNĐ")
    : (document.querySelector(".into-money").textContent =
        resultAll + ".000" + "₫");
...
HandleInformation.ispassword2 = (selector, valueInput, message) => {
  //   if (HandleInformation.ispassword2 !== HandleInformation.ispassword) {
...
if (document.querySelector(".payment-products-container")) {
  document.querySelector(".user-name").innerText =
    users.information.name + " " + users.information.phone;
  document.querySelector(".payment-address").innerText =
    users.information.address +
    ", " +
    users.information.district +
    ", " +
    users.information.city;
  itemPayment();
  ...
  let delivery =
    Number.parseFloat(
      document.querySelector(".payment-delivery-price").innerText
    ) + pricePayment;

  document.querySelector(".total-price-payment").innerText =
    delivery.toLocaleString() + ".000₫";
}

function itemPayment() {
  let htmlpayment = arrItems
    .map((arrItem) => {
      let priceText = Number.parseFloat(arrItem.priceDetai) + "";
...
let productApi =
  "https://ac5f8d67-e14e-487f-b985-4c1254ec6084.mock.pstmn.io/product";
